package igknighters.subsystems.umbrella;
...
import igknighters.util.GamepieceSimulator;
...
public class Umbrella implements LockFullSubsystem {
  ...
  public Umbrella(final SimCtx simRobot) {
   ...
   * @return If the {@code Shooter} is at the target speed
   * @apiNote Tolerance is a percentage of the target speed to allow for error, so a tolerance of
   *     0.1 would allow for a 10% error
   */
  public boolean isShooterAtSpeed(double tolerance) {
    return (Math.abs(shooter.getSpeed() - shooter.getTargetSpeed()) / shooter.getTargetSpeed())
        < tolerance;
  }
   ...
   *
   * @param percent The percent to run the intake at, 1.0 is outtake, -1.0 is intake
  ...
  public void setupSimNoteDetection(Localizer localizer) {
    ...
    GamepieceSimulator.setupGamepieceAcquisitionSim(
            ...
            notes -> {
              Pose2d[] notePoses =
                  notes.stream().map(p -> new Pose2d(p, Rotation2d.kZero)).toArray(Pose2d[]::new);
            ...
            List.of(
                new Transform2d(
                    ...
                    Rotation2d.kZero),
                new Transform2d(
                    new Translation2d(Units.inchesToMeters(24.0), Units.inchesToMeters(-7.0)),
                    Rotation2d.kZero),
                new Transform2d(
                    new Translation2d(Units.inchesToMeters(24.0), Units.inchesToMeters(7.0)),
                    Rotation2d.kZero)))
        .onTrue(Commands.runOnce(() -> intakeSim.setExitBeam(true)).withName("SetExitBeamTrue"));
  }
}
