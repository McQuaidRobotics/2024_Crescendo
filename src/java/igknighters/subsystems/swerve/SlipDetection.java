...
public class SlipDetection implements Logged {
  private static record ModuleData(
      double mps, double distance, Rotation2d angleRads, Translation2d position)
    ...
    public static final Struct<ModuleData> struct =
        ProceduralStructGenerator.genRecord(ModuleData.class);
  }

  @Log
  private final ModuleData[] modules =
      new ModuleData[] {
        ModuleData.zeroed(), ModuleData.zeroed(), ModuleData.zeroed(), ModuleData.zeroed()
      };
  ...
  @Log.Once private final Translation2d[] moduleChassisOffsets = kSwerve.MODULE_CHASSIS_OFFSETS;

  public SlipDetection() {}

  // /**
  //  * Filter the swerve module states to detect slipping and return a better guess of where the
  // robot actually is.
  //  * @param states
  //  * @param positions
  //  * @param acceleration
  //  * @return
  //  */
  // public SwerveModulePosition[] filter(SwerveModuleState[] states, SwerveModulePosition[]
  // positions, double acceleration) {
