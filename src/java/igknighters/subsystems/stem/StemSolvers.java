...
public class StemSolvers {
  private static final TunableBoolean DEBUG_SOLVERS =
      TunableValues.getBoolean("Stem/DebugSolvers", false);
   ...
   *
   * @apiNote When using with the telescope enabled make sure to factor in the current exention of
   *     the telescope in meters into your stem length
  ...
  public static double linearSolveWristTheta(
      double stemLength, double pivotRads, double horizDist, double vertDist, boolean offsetPivot) {

    Translation2d wristLocation = solveWristLocationSimple2d(stemLength, pivotRads);
    double flatWristRads =
        Math.atan((vertDist - wristLocation.getY()) / (horizDist + wristLocation.getX()));
  ...
  public static double gravitySolveWristTheta2(
    ...
    double horizDist =
        targetLocation.toTranslation2d().getDistance(robotLocation) + wrist2dOffset.getX();
   ...
   *
   * @apiNote This is meant to be used with a variant of the stem without a mobile wrist as this
   *     solve is less accurate than a solve involving just varying the theta of the wrist at a
   *     known position.
   * @param stemLength The length of the stem from the pivot axel to the wrist axel
  ...
  public static double linearSolvePivotTheta(
      double stemLength, double wristRads, double horizDist, double vertDist) {
  ...
  /**
   * Calculates the difference between the vertical distance to the target and the distance below
   * the target that a note would land if shot with the given parameters.
   *
   * @param wristRads The angle of the wrist in radians
   ...
   * @return The difference between the vertical distance to the target and the distance below the
   *     target that a note would land if shot with the given parameters
   */
  public static double linearSolveVerticalDistError(
      double wristRads, double horizontalDist, double verticalDist, double deltaNoteVelo) {
   ...
   *
   * @apiNote This can be thought of side view of the robot in cartesian space with the front of the
   *     robot being +X.
  ...
  public static Translation2d solveWristLocationSimple2d(double stemLength, double pivotRads) {
    Translation2d wristLocationVector =
        new Translation2d(stemLength * Math.cos(pivotRads), stemLength * Math.sin(pivotRads));
    return kPivot.kDimensions.PIVOT_AXEL_LOCATION.plus(wristLocationVector);
  }

  /**
   * Solves for the location of the wrist axel in 3d space relative to the center of the robot on
   * the x and y axis and relative to the floor on the z axis.
   *
  ...
  public static Translation3d solveWristLocation3d(double stemLength, double pivotRads) {
    ...
    return new Translation3d(wristAxelLocation.getX(), 0.0, wristAxelLocation.getY());
  }
  ...
  public static double gravitySolveWristTheta(
    ...
    double linearWristRads =
        linearSolveWristTheta(stemLength, pivotRads, horizDist, vertDist, false);
    double vertcialDistanceError =
        linearSolveVerticalDistError(linearWristRads, horizDist, vertDist, deltaNoteVelo);
  ...
  public static StemPosition iterativeSolveLowestPivotDelta(
      double startingPosition, Function<Double, StemPosition> positionGenerator) {
  ...
  public static record StemSolveInput(
      /**
       * This will define the current position of the stem to be used in more optimized solves that
       * desire as little movement as possible.
       */
      StemPosition currentStemPosition,
      /**
       * This will define any static components of a solve like pivot rads for a stationary pivot
       * solve.
       */
      ...
      double vertDist,
      /** The average velocity of the note, this will be used in gravity compensated solves. */
      double deltaNoteVelo) {}
  ...
  public enum AimSolveStrategy {
        ...
        input -> {
          ...
          return StemPosition.fromRadians(
              linearSolvePivotTheta(
                  telescopeMeters, wristRads, input.horizDist(), input.vertDist()),
              wristRads,
              telescopeMeters);
        }),
    STATIONARY_PIVOT(
        input -> {
          ...
          return StemPosition.fromRadians(
              ...
              linearSolveWristTheta(
                  telescopeMeters, pivotRads, input.horizDist(), input.vertDist(), true),
              telescopeMeters);
        }),
    STATIONARY_PIVOT_GRAVITY(
        input -> {
          double pivotRads = input.desiredStemPosition.pivotRads;
          double telescopeMeters = input.desiredStemPosition.telescopeMeters;
          return StemPosition.fromRadians(
    ...
    STATIONARY_PIVOT_GRAVITY_TELESCOPE_EXTEND(
        input -> {
          double pivotRads = input.desiredStemPosition.pivotRads;
    ...
    STATIONARY_PIVOT_TELESCOPE_EXTEND(
        input -> {
          double pivotRads = input.desiredStemPosition.pivotRads;
          double telescopeMeters = StemPosition.INTAKE.telescopeMeters;
          return StemPosition.fromRadians(
