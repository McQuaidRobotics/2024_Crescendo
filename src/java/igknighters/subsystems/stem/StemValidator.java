...
public class StemValidator {
  ...
  private static double rectBottomY(Rectangle2d rect) {
    return rect.getCenter().getY() - (rect.getYWidth() / 2.0);
  }

  private static double rectTopY(Rectangle2d rect) {
    return rect.getCenter().getY() + (rect.getYWidth() / 2.0);
  }

  private static double rectLeftX(Rectangle2d rect) {
    return rect.getCenter().getX() - (rect.getXWidth() / 2.0);
  }

  private static double rectRightX(Rectangle2d rect) {
    return rect.getCenter().getX() + (rect.getXWidth() / 2.0);
  }

  public static class MechanismPoints {
    public final Translation2d wristAxelPoint,
    ...
    public MechanismPoints(StemPosition stemPosition) {
      ...
      Translation2d wristAxelPointTransform =
          new Translation2d(
      ...
      Translation2d umbrellaLengthVector =
          new Translation2d(
              kRobotCollisionGeometry.UMBRELLA_LENGTH
                  * Math.cos(
                      Math.PI
                          - (Math.PI - stemPosition.getWristRads())
      ...
      Translation2d umbrellaTopTransform =
          new Translation2d(
              kRobotCollisionGeometry.UMBRELLA_HEIGHT
                  * Math.cos(
                      stemPosition.getPivotRads() + (Math.PI / 2.0) - stemPosition.getWristRads()),
              kRobotCollisionGeometry.UMBRELLA_HEIGHT
                  * Math.sin(
                      stemPosition.getPivotRads() + (Math.PI / 2.0) - stemPosition.getWristRads()));
    ...
    private double getSmallest(double... values) {
      ...
      for (double value : values) if (value < smallest) smallest = value;
      return smallest;
    }
    ...
    private Pose2d poseFromXY(double x, double y) {
      return new Pose2d(new Translation2d(x, y), Rotation2d.kZero);
    }

    private Rectangle2d getCastedRect() {
      double umbrellaRectLeft =
          getSmallest(
              umbrellaBottomLeftPoint.getX(),
              umbrellaBottomRightPoint.getX(),
              umbrellaTopLeftPoint.getX(),
              umbrellaTopRightPoint.getX());
      double umbrellaRectRight =
          getLargest(
              umbrellaBottomLeftPoint.getX(),
              umbrellaBottomRightPoint.getX(),
              umbrellaTopLeftPoint.getX(),
              umbrellaTopRightPoint.getX());
      double umbrellaRectTop =
          getLargest(
              umbrellaBottomLeftPoint.getY(),
              umbrellaBottomRightPoint.getY(),
              umbrellaTopLeftPoint.getY(),
              umbrellaTopRightPoint.getY());
      double umbrellRectBottom =
          getSmallest(
              umbrellaBottomLeftPoint.getY(),
              umbrellaBottomRightPoint.getY(),
              umbrellaTopLeftPoint.getY(),
      ...
      return new Rectangle2d(poseFromXY(umbrellaRectLeft, umbrellRectBottom), width, height);
    }
  ...
  public enum ValidationResponse implements StructSerializable {
     ...
     */
    private static ValidationResponse collisionFrom(
        boolean drivebaseCollides, boolean boundsCollide) {
    ...
    public static final Struct<ValidationResponse> struct =
        ProceduralStructGenerator.genEnum(ValidationResponse.class);
  }

  /**
   * Returns the intersection of a line and a horizontal line at the given y value.
   *
  ...
  /**
   * Returns true if the passed in x value is inebtween the two planes that make the verticle lines
   * of the side of the rectangle.
   */
  private static boolean withinXBoundsOf(Rectangle2d rect, double x) {
    return x >= rectLeftX(rect) && x <= rectRightX(rect);
  }
  ...
  public static boolean isMechanicallyViable(StemPosition stemPosition, double buffer) {
    if (MathUtil.clamp(
            stemPosition.pivotRads,
            kPivot.MIN_ANGLE * (1.0 - buffer),
            kPivot.MAX_ANGLE * (1.0 + buffer))
        != stemPosition.pivotRads) return false;
    if (MathUtil.clamp(
            stemPosition.wristRads,
            kWrist.MIN_ANGLE * (1.0 - buffer),
            kWrist.MAX_ANGLE * (1.0 + buffer))
        != stemPosition.wristRads) return false;
    if (MathUtil.clamp(
            stemPosition.telescopeMeters,
            kTelescope.MIN_METERS * (1.0 - buffer),
            kTelescope.MAX_METERS * (1.0 + buffer))
        != stemPosition.telescopeMeters) return false;
    return true;
  }

  /**
   * Returns an {@link InvalidationResponse} that defines if the position is valid and if it's not,
   * how its invalid.
   */
  public static ValidationResponse validatePosition(StemPosition stemPosition) {
    ...
    double wristDriveBaseTopIntersect =
        computeIntersection(
            mechPts.umbrellaBottomLeftPoint,
            new Translation2d(
                mechPts.umbrellaBottomRightPoint.getX() + SMOL,
                mechPts.umbrellaBottomRightPoint.getY() + SMOL),
            rectTopY(kRobotCollisionGeometry.DRIVE_BASE));

    double wristDriveBaseBottomIntersect =
        computeIntersection(
            mechPts.umbrellaBottomLeftPoint,
            new Translation2d(
                mechPts.umbrellaBottomRightPoint.getX() + SMOL,
                mechPts.umbrellaBottomRightPoint.getY() + SMOL),
            rectBottomY(kRobotCollisionGeometry.DRIVE_BASE));

    // The x coordinates of where on the top and bottom of the drive base's
    // y coordinates intesects with the function of the line of the angle
    // of the pivot with the stem length.
    double stemDriveBaseTopIntersect =
        computeIntersection(
            new Translation2d(
                kRobotCollisionGeometry.PIVOT_LOCATION.getX() + SMOL,
                kRobotCollisionGeometry.PIVOT_LOCATION.getY() + SMOL),
            mechPts.wristAxelPoint,
            rectTopY(kRobotCollisionGeometry.DRIVE_BASE));

    double stemDriveBaseBottomIntersect =
        computeIntersection(
            new Translation2d(
                kRobotCollisionGeometry.PIVOT_LOCATION.getX() + SMOL,
                kRobotCollisionGeometry.PIVOT_LOCATION.getY() + SMOL),
    ...
    boolean topUmbrellaInterceptInRect =
        withinXBoundsOf(kRobotCollisionGeometry.DRIVE_BASE, wristDriveBaseTopIntersect);
    boolean bottomUmbrellaIntrceptInRect =
        withinXBoundsOf(kRobotCollisionGeometry.DRIVE_BASE, wristDriveBaseBottomIntersect);
    boolean isUmbrellaRightPointsBelowDriveBase =
        (mechPts.umbrellaBottomRightPoint.getY() <= rectTopY(kRobotCollisionGeometry.DRIVE_BASE)
            || mechPts.umbrellaTopRightPoint.getY()
                <= rectTopY(kRobotCollisionGeometry.DRIVE_BASE));

    boolean topStemInterceptInRect =
        withinXBoundsOf(kRobotCollisionGeometry.DRIVE_BASE, stemDriveBaseTopIntersect);
    boolean bottomStemInterceptInRect =
        withinXBoundsOf(kRobotCollisionGeometry.DRIVE_BASE, stemDriveBaseBottomIntersect);
    boolean isWristAxelPointBelowDrivebase =
        mechPts.wristAxelPoint.getY() <= rectTopY(kRobotCollisionGeometry.DRIVE_BASE);

    boolean umbrellaCollidesDrivebase =
        isUmbrellaRightPointsBelowDriveBase
            && (topUmbrellaInterceptInRect || bottomUmbrellaIntrceptInRect);
    boolean stemCollidesDrivebase =
  ...
  /** Will */
  public static StemPosition stepTowardsTargetPosition(
      StemPosition currentState, StemPosition targetState) {
    ValidationResponse pivotMovementValidReason =
        validatePosition(
            StemPosition.fromRadians(
                targetState.getPivotRads(),
                currentState.getWristRads(),
                currentState.getTelescopeMeters()));
    ValidationResponse wristMovementValidReason =
        validatePosition(
            StemPosition.fromRadians(
                currentState.getPivotRads(),
                targetState.getWristRads(),
                currentState.getTelescopeMeters()));
    ValidationResponse telescopeMovementValidReason =
        validatePosition(
            StemPosition.fromRadians(
                currentState.getPivotRads(),
                currentState.getWristRads(),
    ...
    double midStatePivotRads =
        pivotMovementValidReason.isValid()
            ? targetState.getPivotRads()
            : currentState.getPivotRads();
    double midStateWristRads =
        wristMovementValidReason.isValid()
            ? targetState.getWristRads()
            : currentState.getWristRads();
    double midStateTelescopeMeters =
        telescopeMovementValidReason.isValid()
            ? targetState.getTelescopeMeters()
            : currentState.getTelescopeMeters();

    if (!pivotMovementValidReason.isValid()
        && !pivotMovementValidReason.equals(ValidationResponse.NOT_MECHANICALLY_VIABLE)) {
      StemPosition invalidPosition =
          StemPosition.fromRadians(
              targetState.getPivotRads(),
              currentState.getWristRads(),
              currentState.getTelescopeMeters());
      ...
      if (!pivotMovementValidReason.equals(ValidationResponse.COLLIDES_BOUNDS)) {
        ...
        double lowestOfUmbrellaPoints =
            Math.min(mechPts.umbrellaBottomRightPoint.getY(), mechPts.umbrellaTopRightPoint.getY());
        double difference =
            Math.abs(lowestOfUmbrellaPoints - rectTopY(kRobotCollisionGeometry.DRIVE_BASE));
        double opposite =
            (currentState.telescopeMeters * Math.sin(invalidPosition.getPivotRads())) + difference;
        ...
        double rightDiff = diffIfGreater(rectRightX(umbrellaRect), rectRightX(bounds));
        double leftDiff = diffIfGreater(rectLeftX(bounds), rectLeftX(umbrellaRect));
        double topDiff = diffIfGreater(rectTopY(umbrellaRect), rectTopY(bounds));
        double bottomDiff = diffIfGreater(rectBottomY(bounds), rectBottomY(umbrellaRect));

        Translation2d midWristAxelPoint =
            mechPts.wristAxelPoint.plus(
                new Translation2d(leftDiff - rightDiff, bottomDiff - topDiff));

        Translation2d pivotAxelToWristAxelSlope =
            midWristAxelPoint.minus(kRobotCollisionGeometry.PIVOT_LOCATION);

        midStateTelescopeMeters =
            Math.sqrt(
                Math.pow(pivotAxelToWristAxelSlope.getX(), 2)
                    + Math.pow(pivotAxelToWristAxelSlope.getY(), 2));
        midStatePivotRads = Math.asin(pivotAxelToWristAxelSlope.getY() / midStateTelescopeMeters);
      }
    }
