 ...
 *
 * <p>The forward limit switch is an IR beam break sensor that is used to detect when the telescope
 * is fully extended. This is unreliable in the sun and thus has to be disabled when using the robot
 * outside.
 */
public class TelescopeRealSunshine extends Telescope {
  ...
  public TelescopeRealSunshine() {
    ...
    CANSignalManager.registerSignals(
        kStem.CANBUS, motorRots, motorVelo, motorVolts, motorAmps, reverseLimitSwitch);
  ...
  private TalonFXConfiguration motorConfig() {
    ...
    cfg.HardwareLimitSwitch.ForwardLimitAutosetPositionValue =
        mechMetersToMotorRots(kTelescope.MAX_METERS);
    ...
    cfg.HardwareLimitSwitch.ReverseLimitAutosetPositionValue =
        mechMetersToMotorRots(kTelescope.MIN_METERS);
  ...
  private double mechMetersToMotorRots(Double mechMeters) {
    return ((mechMeters - kTelescope.MIN_METERS) / kTelescope.SPROCKET_CIRCUMFERENCE)
        * kTelescope.MOTOR_TO_MECHANISM_RATIO;
  }

  private double motorRotsToMechMeters(Double motorRots) {
    return ((motorRots / kTelescope.MOTOR_TO_MECHANISM_RATIO) * kTelescope.SPROCKET_CIRCUMFERENCE)
  ...
  public void setVoltageOut(double volts) {
    ...
    this.motor.setControl(controlReqVolts.withOutput(volts));
  }

  @Override
  public void gotoPosition(double meters) {
    super.targetMeters = meters;
    this.motor.setControl(controlReqMotionMagic.withPosition(mechMetersToMotorRots(meters)));
  }
  ...
  public void setCoast(boolean shouldBeCoasting) {
    this.motor.setNeutralMode(shouldBeCoasting ? NeutralModeValue.Coast : NeutralModeValue.Brake);
  }

  @Override
  public void periodic() {
    FaultManager.captureFault(
        StemHW.TelescopeMotor, motorRots, motorVelo, motorVolts, motorAmps, reverseLimitSwitch);
