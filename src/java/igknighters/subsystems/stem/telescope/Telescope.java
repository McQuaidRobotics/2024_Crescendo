...
public abstract class Telescope extends Component {
   ...
   * @apiNote This is distance from pivot axel to the wrist axel, this means 0.0 is not fully
   *     retraccted but rather an unreachable position. Between {@value
   *     ConstValues.kStem.kTelescope.MIN_METERS} and {@value
   *     ConstValues.kStem.kTelescope.MAX_METERS} for the min and max.
   */
  ...
  /**
   * Move the telescope to the target and returns if it has reached the target. Meant to be used in
   * a kind of polling loop to wait the mechanism to reach the target.
   *
   * @param meters The target distance to move to
   ...
   * @param toleranceMult The multiplier to apply to the tolerance, higher mult means more tolerance
  ...
  public boolean isAt(double radians, double toleranceMult) {
    return Math.abs(this.getPosition() - radians)
        < ConstValues.kStem.kTelescope.TARGET_TOLERANCE * toleranceMult;
  }
