...
import igknighters.subsystems.stem.wrist.*;
...
public class Stem implements LockFullSubsystem {
  ...
  public Stem(final SimCtx simRobot) {
  ...
  /**
   * Meant as the main entry point for controlling the stem, this method takes in a {@link
   * StemPosition} and sets the stem to that position. This method will return false if any of the
  ...
  public boolean gotoStemPosition(StemPosition position, double toleranceMult) {
    ...
    if (!validity.isValid()) {
      DriverStation.reportError(
          "Invalid TARGET stem position(" + validity.name() + "): " + position.toString(), true);
      return true;
    }
    ...
    if (!telescope.hasHomed()) {
      ...
      boolean wristAndPivot =
          pivot.target(position.pivotRads, 1.0) && wrist.target(position.wristRads, 1.0);
  ...
  /**
   * Meant as the main api for controlling the stem, this method takes in a {@link StemPosition} and
   * sets the stem to that position. This method will return false if any of the mechanisms have not
   * yet reached their target position.
   *
   * @param position The position to set the stem to
   * @return True if all mechanisms have reached their target position
   */
  ...
  /**
   * Will check if all mechanisms have reached their target position within a specific tolerance.
   *
   * @param targetPosition The desired position
   * @param toleranceMult A value to multiply the accepted positional tolerance by @@return True if
   *     all mechanisms have reached their target position
   */
   ...
   * Will check if all mechanisms have reached their target position within a specific tolerance.
   *
   * @param targetPosition The desired position @@return True if all mechanisms have reached their
   *     target position
   */
  ...
  public StemPosition getStemPosition() {
    return StemPosition.fromRadians(
        pivot.getPosition(), wrist.getPosition(), telescope.getPosition());
  }

  /**
   * Immedaitely cuts off the voltage to all mechanisms causing them to stop, this is not
  ...
  /**
   * Control each component with voltage control. This is NOT recommended for general use and should
   * only be used to test the mechanisms.
   *
  ...
  public double[] getStemVelocities() {
    return new double[] {pivot.getVelocity(), telescope.getVelocity(), wrist.getVelocity()};
  }
  ...
  /**
   * Returns if the stem is OK, the stem can not be OK for a variety of reasons such as the
   * telescope not being homed, hardware being dropped, or the stem being in an invalid position.
   */
  public boolean isOk() {
    return StemValidator.validatePosition(getStemPosition()).isValid() && telescope.hasHomed();
  }
  ...
  public SysIdRoutine pivotSysidRoutine() {
    return new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism(v -> pivot.setVoltageOut(v.baseUnitMagnitude()), l -> {}, this));
  }
}
