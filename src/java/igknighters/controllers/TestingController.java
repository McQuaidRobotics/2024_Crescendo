...
public class TestingController extends ControllerBase {
  private static final TunableDouble passDistance =
      TunableValues.getDouble("Test/PassTestDist", 11.0);
  ...
  public TestingController(int port, Localizer localizer) {
    ...
    this.B.binding =
        new Binding(
            Subsystems.Umbrella,
            (trig, allss) -> {
              trig.onTrue(UmbrellaCommands.shoot(allss.umbrella.get(), () -> 1000));
            });
    ...
    this.X.binding =
        new Binding(
            Subsystems.Stem,
            (trig, allss) -> {
              trig.onTrue(UmbrellaCommands.expell(allss.umbrella.get()));
            });
        ...
        new Binding(
            ...
            (trig, allss) -> {
              trig.onTrue(
                  StemCommands.moveTo(
                      allss.stem.get(),
                      StemPosition.fromDegrees(41.0, 108.0, kTelescope.MIN_METERS)));
            });
            ...
            (trig, allss) -> {
              ...
              trig.whileTrue(
                  Commands.deadline(
                      ...
                      .andThen(new ScheduleCommand(StemCommands.holdStill(stem)))
                      .withName("idk"));
            },
            Subsystems.Stem,
            Subsystems.Umbrella);
            ...
            (trig, allss) -> {
              ...
              trig.onTrue(
                  Commands.defer(
                      ...
                      .andThen(
                          Commands.defer(() -> Commands.print("Time: " + timer.get()), Set.of()))
                      .andThen(timer::stop));
            },
            Subsystems.Swerve);
    ...
    // LED.sendAnimation(LedAnimations.TELEOP)).ignoringDisable(true));
    // });

    // this.DPD.binding = new Binding((trig, allss) -> {
    //     trig.onTrue(Commands.runOnce(() ->
    // LED.sendAnimation(LedAnimations.ERROR)).ignoringDisable(true));
    // });

    // this.DPL.binding = new Binding((trig, allss) -> {
    //     trig.onTrue(Commands.runOnce(() ->
    // LED.sendAnimation(LedAnimations.SHOOTING)).ignoringDisable(true));
    // });

    // this.DPU.binding = new Binding((trig, allss) -> {
    //     trig.onTrue(Commands.runOnce(() ->
    // LED.sendAnimation(LedAnimations.INTAKE)).ignoringDisable(true));
    // });
  }
}
