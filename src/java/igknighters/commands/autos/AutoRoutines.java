...
public class AutoRoutines extends AutoCommands {
  ...
  public AutoRoutines(AllSubsystems allSubsystems, Localizer localizer, AutoFactory factory) {
    super(
        ...
        allSubsystems.stem.get(),
        allSubsystems.umbrella.get(),
        allSubsystems.vision.get(),
        localizer);
  ...
  /**
   * Shoots starting note, Tries to pickup C1 and shoots it if we have it, Picks up M1 and shoots it
   * if we have it, If we don't have M1, picks up M2 and shoots it if we have it, If we don't have
   * M2, picks up M3 and shoots it if we have it, After we shoot the second note, we pickup C2 and
   * shoot it if we have it. Finally we pickup C3 and shoot it.
   *
            ...
            ampToC1
                ...
                .andThen(
                    ...
                    Commands.parallel(
                        ...
                        Commands.waitSeconds(0.2).andThen(new ScheduleCommand(ampToC1.cmd()))))
  ...
  public AutoRoutine sixPieceFarAmpSide() {
    ...
    // picking up the third note, only shooting if we have a note
    s1ToC2.active().or(s2ToC2.active()).onTrue(intakeGamepieceNoStow());
    s1ToC2.done().or(s2ToC2.done()).and(yeGp(routine)).onTrue(c2ToSUB.cmd());
    ...
    routine
        ...
        .onTrue(
            srcToM5
                .resetOdometry()
                .andThen(
                    autoShootBeginning(), Commands.parallel(intakeGamepieceNoStow(), srcToM5.cmd()))
