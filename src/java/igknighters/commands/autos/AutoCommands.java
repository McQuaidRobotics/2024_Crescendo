...
import igknighters.commands.swerve.teleop.AutoSwerveTargetSpeakerCmd;
...
public class AutoCommands {
  ...
  protected AutoCommands(
      Swerve swerve, Stem stem, Umbrella umbrella, Vision vision, Localizer localizer) {
    this.swerve = swerve;
    this.stem = stem;
  ...
  protected Command stow() {
    return loggedCmd(StemCommands.moveTo(stem, StemPosition.STOW).withName("Stow"));
  }

  protected Command aimSub() {
    return loggedCmd(StemCommands.moveTo(stem, StemPosition.STARTING).withName("AimSub"));
  }
  ...
  protected Command autoShoot(AutoRoutine routine) {
    return loggedCmd(
        Commands.parallel(
            ...
            .andThen(feedShooter().finallyDo(() -> logAutoEvent("Shooting", "Done")))
    ...
    return loggedCmd(
        Commands.parallel(
                ...
                StemCommands.aimAtSpeaker(
                    ...
                    .finallyDo(() -> logAutoEvent("Stem Targeting", "Done")))
            .andThen(
                feedShooter().finallyDo(() -> logAutoEvent("Shooting", "Done")),
