...
public class StemCommands {
  ...
  /**
   * A command class that continually calculates the wrist radians needed to pass a gamepiece to a
   * position on then field
   */
  private static class AimAtPassPointCommand extends Command {
    ...
    private AimAtPassPointCommand(
        Stem stem, Translation2d passPoint, boolean canFinish, Supplier<Pose2d> poseSupplier) {
      addRequirements(stem);
      this.stem = stem;
    ...
    public void initialize() {
      targetTranslation =
          AllianceFlip.isBlue() ? passPoint : AllianceFlip.flipTranslation(passPoint);
    }
    ...
    public void execute() {
      ...
      double wristRads =
          StemSolvers.gravitySolveWristTheta2(
              telescopeMeters,
              pivotRads,
              currentPose.getTranslation(),
              new Translation3d(targetTranslation.getX(), targetTranslation.getY(), 0.0),
              noteMps.value(),
              false,
              true);

      stem.gotoStemPosition(StemPosition.fromRadians(pivotRads, wristRads, telescopeMeters));
    }
  ...
  /**
   * Will move the stem to a position and finish when it has reached the desired position.
   *
  ...
  public static Command holdAt(Stem stem, StemPosition pose) {
    return stem.run(() -> stem.gotoStemPosition(pose, 0.0)).withName("Hold Stem(" + pose + ")");
  }
  ...
  public static Command aimAtSpeaker(
      ...
      boolean canFinish,
      Supplier<Pose2d> poseSupplier,
      Supplier<FieldSpeeds> veloSupplier) {
    return aimAtSpeaker(
        stem, kControls.DEFAULT_AIM_STRATEGY, canFinish, poseSupplier, veloSupplier);
  }

  /**
   * Aims the pivot or wrist or both depending on the default aim strategy in constants.
   *
   * @param stem The stem subsystem
   ...
   * @return
   */
  public static Command aimAtPassPoint(
      Stem stem, Translation2d passPoint, boolean canFinish, Supplier<Pose2d> poseSupplier) {
    return new AimAtPassPointCommand(stem, passPoint, canFinish, poseSupplier)
        .withName(
            String.format("AimAtPassPoint(x: %.2f, y: %.2f)", passPoint.getX(), passPoint.getY()));
  }
