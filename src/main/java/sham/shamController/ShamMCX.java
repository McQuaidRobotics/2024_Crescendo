...
public class ShamMCX implements ShamMotorController {
  ...
  private static final VelocityUnit<AngularVelocityUnit> AU = VelocityUnit.combine(RadiansPerSecond, Seconds);

  private sealed interface Output {
    public ControllerOutput run(Time dt, Voltage supply, MechanismState state, EpilogueBackend logger);
      ...
      public ControllerOutput run(Time dt, Voltage supply, MechanismState state, EpilogueBackend logger) {
        Measure<?> output;
          if (controller().isVelocity()) {
            ...
            output = controller().runPosition(
              state.position(),
              State.of(state.position(), state.velocity()),
              new State<>(
                (Measure<AngleUnit>) value,
                (Velocity<AngleUnit>) secondOrderValue
              ),
              dt,
              logger
            );
  ...
  public record CurrentLimits(
      ...
      Time lowerLimitTriggerTime) implements StructSerializable {
    ...
    public static final Struct<CurrentLimits> struct = ProceduralStructGenerator.genRecord(CurrentLimits.class);
  }
  ...
  public enum SoftLimitTrigger {
    ...
    public static final Struct<SoftLimitTrigger> struct = ProceduralStructGenerator.genEnum(SoftLimitTrigger.class);
  }
  ...
  public void controlCurrent(ClosedLoop<CurrentUnit, AngleUnit, AngleUnit> controller, Angle position, AngularVelocity velocity) {
    ...
    output = Optional.of(new Output.ClosedLoopOutput<>(controller, position, VU.of(velocity.baseUnitMagnitude())));
  }

  public void controlCurrent(ClosedLoop<CurrentUnit, AngleUnit, AngleUnit> controller, Angle position) {
    controlCurrent(controller, position, RadiansPerSecond.zero());
  }

  public void controlCurrent(ClosedLoop<CurrentUnit, AngularVelocityUnit, AngleUnit> controller, AngularVelocity velocity, AngularAcceleration acceleration) {
    ...
    output = Optional.of(new Output.ClosedLoopOutput<>(controller, velocity, AU.of(acceleration.baseUnitMagnitude())));
  }

  public void controlCurrent(ClosedLoop<CurrentUnit, AngularVelocityUnit, AngleUnit> controller, AngularVelocity velocity) {
  ...
  public void controlVoltage(ClosedLoop<VoltageUnit, AngleUnit, AngleUnit> controller, Angle position, AngularVelocity velocity) {
    if (controller == null || position == null) {
      output = Optional.empty();
      return;
    }
  ...
  public void controlVoltage(ClosedLoop<VoltageUnit, AngleUnit, AngleUnit> controller, Angle position) {
    controlVoltage(controller, position, RadiansPerSecond.zero());
  }

  public void controlVoltage(ClosedLoop<VoltageUnit, AngularVelocityUnit, AngleUnit> controller, AngularVelocity velocity, AngularAcceleration acceleration) {
    if (controller == null || velocity == null) {
      output = Optional.empty();
      return;
    }
  ...
  public void controlVoltage(ClosedLoop<VoltageUnit, AngularVelocityUnit, AngleUnit> controller, AngularVelocity velocity) {
  ...
  private ControllerOutput currentLimit(Time dt, ControllerOutput requestedOutput, MechanismState state, Voltage supplyVoltage) {
    ...
    boolean overLowerSupplyLimit = supplyCurrent.gt(limits.supplyCurrentLowerLimit)
