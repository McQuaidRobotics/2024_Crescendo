...
public class StemValidator {
    ...
    public static class MechanismPoints {
        ...
        public MechanismPoints(StemPosition stemPosition) {
            ...
            Translation2d umbrellaLengthVector = new Translation2d(
                    kRobotGeometry.UMBRELLA_LENGTH
                            * Math.cos(Math.PI - (Math.PI - stemPosition.getWristRads()) - stemPosition.getPivotRads()),
                    kRobotGeometry.UMBRELLA_LENGTH * -1.0
                            * Math.sin(
                                    Math.PI - (Math.PI - stemPosition.getWristRads()) - stemPosition.getPivotRads()));
        ...
        private Rectangle2d getCastedRect() {
            double umbrellaRectLeft = getSmallest(umbrellaBottomLeftPoint.getX(), umbrellaBottomRightPoint.getX(),
                    umbrellaTopLeftPoint.getX(), umbrellaTopRightPoint.getX());
            double umbrellaRectRight = getLargest(umbrellaBottomLeftPoint.getX(), umbrellaBottomRightPoint.getX(),
                    umbrellaTopLeftPoint.getX(), umbrellaTopRightPoint.getX());
            double umbrellaRectTop = getLargest(umbrellaBottomLeftPoint.getY(), umbrellaBottomRightPoint.getY(),
     ...
     */
    public static ValidationResponse isValidPosition(StemPosition stemPosition) {
        ...
        boolean topUmbrellaInterceptInRect = withinXBoundsOf(kRobotGeometry.DRIVE_BASE, wristDriveBaseTopIntersect);
    ...
    public static StemPosition stepTowardsTargetPosition(StemPosition currentState, StemPosition targetState) {
        ...
        ValidationResponse wristMovementValidReason = isValidPosition(StemPosition.fromRadians(
                currentState.getPivotRads(),
                targetState.getWristRads(),
                currentState.getTelescopeMeters()));
        ValidationResponse telescopeMovementValidReason = isValidPosition(StemPosition.fromRadians(
                currentState.getPivotRads(),
                currentState.getWristRads(),
        ...
        if (!pivotMovementValidReason.isValid()) {
            ...
            if (!pivotMovementValidReason.equals(ValidationResponse.COLLIDES_BOUNDS)) {
                ...
                double difference = Math.abs(lowestOfUmbrellaPoints - kRobotGeometry.DRIVE_BASE.getTopY());
                ...
                midStateTelescopeMeters = Math.sqrt(
                        Math.pow(pivotAxelToWristAxelSlope.getX(), 2) + Math.pow(pivotAxelToWristAxelSlope.getY(), 2));
