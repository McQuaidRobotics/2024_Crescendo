...
public class SwerveModuleSim3 extends SwerveModule {
    ...
    public SwerveModuleSim3(final int moduleId, SimSwerveOdometryThread odoThread, ShamSwerve sim) {
        ...
        driveLoop = ClosedLoop.forVoltageAngularVelocity(
        ...
        steerLoop = ClosedLoop.forVoltageAngle(
    ...
    public SwerveModulePosition getCurrentPosition() {
        return new SwerveModulePosition(
            driveMotor.position().div(kSwerve.DRIVE_GEAR_RATIO).in(Rotations) * kSwerve.WHEEL_CIRCUMFERENCE,
    ...
    public SwerveModuleState getCurrentState() {
        return new SwerveModuleState(
            driveMotor.velocity().div(kSwerve.DRIVE_GEAR_RATIO).in(RotationsPerSecond) * kSwerve.WHEEL_CIRCUMFERENCE,
            new Rotation2d(steerMotor.position())
        );
    }
    ...
    private void setSpeed(SwerveModuleState desiredState) {
        ...
        driveMotor.controlVoltage(
            driveLoop,
            RotationsPerSecond.of((desiredState.speedMetersPerSecond / kSwerve.WHEEL_CIRCUMFERENCE) * kSwerve.DRIVE_GEAR_RATIO)
        );
    }
    ...
    public void periodic() {
        ...
        super.drivePositionMeters = driveMotor.position().div(kSwerve.DRIVE_GEAR_RATIO).in(Rotations) * kSwerve.WHEEL_CIRCUMFERENCE;
        super.driveVeloMPS = driveMotor.velocity().div(kSwerve.DRIVE_GEAR_RATIO).in(RotationsPerSecond) * kSwerve.WHEEL_CIRCUMFERENCE;
