 ...
 * 
 * <p> Speeds can come in two variants for different
 * <a href="https://docs.wpilib.org/en/stable/docs/software/basic-programming/coordinate-system.html">frame of references</a>:
 * <ul>
...
public sealed interface Speeds extends StructSerializable {
     ...
     * <li> (0, 0) is the corner between the blue alliance wall and the wall adjacent in the -y direction.
     * </ul>
     */
    public record FieldSpeeds(double vx, double vy, double omega) implements Speeds {
        ...
        public FieldSpeeds discretize(double dtSeconds) {
            return Internals.discretizePrim(
                vx, vy, omega, dtSeconds, FieldSpeeds::new
            );
        }
    }
    ...
    public record RobotSpeeds(double vx, double vy, double omega) implements Speeds {
        ...
        public RobotSpeeds discretize(double dtSeconds) {
            return Internals.discretizePrim(
                vx, vy, omega, dtSeconds, RobotSpeeds::new
            );
        }
    ...
    static RobotSpeeds fromRobotRelative(ChassisSpeeds speeds) {
        return new RobotSpeeds(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond, speeds.omegaRadiansPerSecond);
    }

    static FieldSpeeds fromFieldRelative(ChassisSpeeds speeds) {
        return new FieldSpeeds(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond, speeds.omegaRadiansPerSecond);
    }
    ...
    class SpeedsStruct implements Struct<Speeds> {
        enum SpeedsType implements StructSerializable {
            ...
            public static final Struct<SpeedsType> struct = ProceduralStructGenerator.genEnum(SpeedsType.class);
        }
